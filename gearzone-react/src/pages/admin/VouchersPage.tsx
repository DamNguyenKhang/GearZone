...
export default function AdminVouchersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false); setForm({ code: '', discountType: 'Percentage', discountValue: '', usageLimit: '', expiresAt: '' });
      fetchVouchers();
    } catch (err: unknown) {
    ...
    <div style={{ padding: '2rem' }}>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ padding: '1.5rem', background: '#f9f9f9', borderRadius: 8, marginBottom: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', maxWidth: 600 }}>
          ...
          <input placeholder="Code" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} required style={{ padding: '0.5rem' }} />
          ...
          <input type="date" value={form.expiresAt} onChange={e => setForm(f => ({ ...f, expiresAt: e.target.value }))} style={{ padding: '0.5rem' }} />
