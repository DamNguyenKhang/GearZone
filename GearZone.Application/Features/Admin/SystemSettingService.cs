...
public class SystemSettingService : ISystemSettingService
{
    ...
    public SystemSettingService(ISystemSettingRepository settingRepository, IUnitOfWork unitOfWork, IMemoryCache cache)
    {
    ...
    public async Task<List<SystemSettingDto>> GetAllSettingsAsync()
    {
        if (!_cache.TryGetValue(CacheKey, out List<SystemSettingDto>? cachedSettings) || cachedSettings == null)
        {
            ...
            cachedSettings = settings.Select(s => new SystemSettingDto
            {
                Key = s.Key,
                Value = s.Value,
                DataType = s.DataType,
